...
# 8-Puzzle Solver using Breadth-First Search (BFS) in C++
...
This program implements a Breadth-First Search (BFS) to solve the 8-puzzle problem. It systematically explores possible board configurations using valid tile movements (up, down, left, right).

The search maintains:
- A **queue of paths** being explored (FIFO for BFS).
- A **visited list** to prevent revisiting previously seen board states.
- A **final path stack** to store the solution sequence once the goal state is found.

Each element in the queue is a path represented as a stack of 2D board states (from initial to current state).
...
1. **Initialization**
    - The goal state is defined statically.
    - The initial state is pushed into a stack and enqueued into the main BFS queue.
    - It is also added to the list of visited states.

2. **Breadth-First Search Loop**
    - While the queue is not empty:
        - Dequeue a path (stack of states).
        - If the top state matches the goal, the solution has been found.
        - Otherwise, generate all valid next moves from the current state.
        - For each valid move:
            - If the resulting state has not been visited:
                - Clone the current path, push the new state, and enqueue the new path.
                - Add the new state to the visited list.
...
Queue size: 3
Goal state reached!
Final Stack (Path from initial to goal):
1 2 3
4 5 6
7 0 8
-----
...
### Time Complexity
- Worst-case time complexity: `O(b^d) ≈ O(4^d)`
- Since BFS explores level by level, it guarantees the shortest path to the goal.
- Upper bound is `O(n)` but pruning with the visited list reduces actual search.

### Space Complexity
- Space used for:
    - The `mainQueue` of paths: `O(b^d)` in the worst case.
    - The `Visited` list: `O(n)`.
    - Each path stack contains `O(d)` board states.

---
## Limitations
- BFS guarantees the shortest path but consumes a large amount of memory due to storing many states at each level.
- Can be infeasible for very deep puzzles.
